import { $, component$, useOnWindow, useSignal, useContext } from "@builder.io/qwik";

import { useLocalStorage } from "~/hooks/useLocalStorage";
import { LocaleContext } from "~/store/locale-store";
import { translateSectionTitle, translateSectionDescription } from "~/i18n/section-translations";
          ...
          <div class="flex-shrink-0 flex flex-col py-4 h-auto items-stretch justify-evenly">
            ...
            {(done.value && done.value[index]) ? (
              <p class={`text-${section.color}-400 pt-2 pb-0 px-0 mx-0 my-0`}>
                {done.value[index]}/{section.checklist.length} {t.sectionLinkGrid.done}
              </p>
            ) : (
              <p class={`text-${section.color}-400 pt-2 pb-0 px-0 mx-0 my-0`}>
                {section.checklist.length} {t.sectionLinkGrid.items}
              </p>
            )}
          </div>
          <div class="card-body flex-grow py-2 pl-4 pr-0">
            <h2 class={`card-title text-${section.color}-400 hover:text-${section.color}-500 text-sm`}>
              {translateSectionTitle(section.title, locale)}
            </h2>
            <p class="p-0">{translateSectionDescription(section.title, locale, section.description)}</p>
            {(completions.value && completions.value[index]) ? (
              ...
              <span class="absolute right-2 top-2 opacity-30 text-xs">
                {t.sectionLinkGrid.notYetStarted}
              </span>
            )}
          </div>
        </a>
