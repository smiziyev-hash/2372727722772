...
export default component$(() => {
   ...
   * @param currentLocale - The current locale for translations
  ...
  // Store chart instance to update it when locale changes
  const chartInstance = useSignal<any>(null);
  const chartRegistered = useSignal(false);

  const createRadarChart = $(() => {
    // Register Chart.js only once
    if (!chartRegistered.value) {
      Chart.register(...registerables);
      chartRegistered.value = true;
    }

    // Priority label translations
    const priorityLabels: Record<Priority, string> = {
      'essential': locale === 'ru' ? t.common.basic : 'Essential',
      'optional': locale === 'ru' ? t.common.optional : 'Optional',
    ...
    makeRadarData(sections, locale, priorityLabels).then((data) => {
      if (radarChart.value) {
        ...
        const completedText = locale === 'ru' ? 'Выполнено' : 'Completed';
        const ofText = locale === 'ru' ? 'из' : 'of';
        const itemsText = locale === 'ru' ? 'элементов' : 'items';
        ...
        chartInstance.value = new Chart(radarChart.value, {
          ...
          options: {
            ...
            plugins: {
              ...
              tooltip: {
                callbacks: {
                  label: (ctx) => `${completedText} ${Math.round(ctx.parsed.r)}% ${ofText} ${ctx.dataset.label || ''} ${itemsText}`,
                }
              }
            },
          }
  ...
  useOnWindow('load', createRadarChart);
  ...
  const items = [
    { id: 'essential-container', label: t.common.basic },
    { id: 'optional-container', label: t.common.optional },
    { id: 'advanced-container', label: t.common.advanced },
  ];
  ...
  <div class="flex justify-center flex-wrap items-stretch gap-6 mb-4 relative">
    ...
    <div class="
      ...
      backdrop-filter backdrop-blur-md bg-opacity-40 border border-stone-800">
        ...
        <p class="text-xl block text-center font-bold">{t.progress.noStatsYet}</p>
        <p class="w-md text-left my-2">{t.progress.noStatsDescription}</p>
        <p class="w-md text-left my-2">{t.progress.getStarted}</p>
      </div>
    )}
    ...
    <div class="flex justify-center flex-col items-center gap-6">
      ...
      <div class="rounded-box bg-front shadow-md w-96 p-4">
        ...
        <p class="text-lg">
          {t.progress.completedOutOf} <b>{totalProgress.value.completed} {t.progress.completedOutOfText} {totalProgress.value.outOf}</b> {locale === 'ru' ? 'элементов' : t.common.items}
        </p>
        <progress
          class="progress w-80"
      ...
      {items.map((item) => (
        <div
          key={item.id}
          class="flex flex-col justify-center items-center carousel-item w-20 p-4
      ...
      <div class="p-4 rounded-box bg-front shadow-md w-96 flex-grow">
        ...
        <a 
          ...
          href={t.progress.viewDirectory} 
          target="_blank" 
          rel="noopener noreferrer"
        >
          {t.progress.nextUpDescription}
        </a>
      </div>
    </div>
          ...
          { (Array.isArray(checklists.value) ? checklists.value : []).map((section: Section, index: number) => (
              <li key={index}>
                ...
                <p class="text-sm m-0 flex items-center text-left gap-1 text-nowrap overflow-hidden max-w-40">
                  ...
                  {translateSectionTitle(section.title, locale)}
                </p>
