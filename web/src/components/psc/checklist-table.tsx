  ...
  return (
    ...
    <div class="flex flex-wrap justify-between items-center">
      <div>
        ...
        <p class="text-xs text-center">
          {done} {localeSignal.value === 'ru' ? 'из' : 'out of'} {total} ({percent}%)
          {t.checklistTable.complete}, {disabled} {t.checklistTable.ignored}</p>
      </div>
      ...
      <div class="flex flex-wrap gap-2 justify-end my-4">
        ...
        <button class="btn btn-sm hover:btn-primary" onClick$={() => { showFilters.value = !showFilters.value; }}>
          ...
          {showFilters.value ? t.checklistTable.hideFilters : t.checklistTable.showFilters}
        </button>
      </div>
    </div>
    ...
    {showFilters.value && (
      <div class="flex flex-wrap justify-between bg-base-100 rounded px-4 py-1 transition-all"
        ...
        <div class="flex justify-end items-center gap-1">
          ...
          <label class="p-2 rounded hover:bg-front transition-all cursor-pointer flex gap-2">
            <span class="text-sm">{t.checklistTable.optional}</span> 
            <input
              type="checkbox"
          ...
          <label
            class="p-2 rounded hover:bg-front transition-all cursor-pointer flex gap-2">
            <span class="text-sm">{t.checklistTable.advanced}</span> 
            <input
              type="checkbox"
    ...
    <table class="table">
      <thead>
        <tr>
          { [
            { id: 'done', text: t.checklistTable.done},
            { id: 'advice', text: t.checklistTable.advice },
            { id: 'level', text: t.checklistTable.level }
          ].map((item) => (
            <th
              key={item.id}
          ...
          <th>{t.checklistTable.details}</th>
        </tr>
      </thead>
      <tbody>
          ...
          return (
            <tr key={index} class={[
              ...
              <td class="text-center">
                ...
                />
                <label for={`ignore-${itemId}`} class="text-small block opacity-50 mt-2">{t.common.ignore}</label>
                <input
                  type="checkbox"
                  id={`ignore-${itemId}`}
              ...
              <td>
                <label
                  ...
                  {locale === 'ru' && props.section.slug
                    ? translateChecklistContent(props.section.slug, item.point, locale, 'point', item.point) || item.point
                    : item.point}
                </label>
              </td>
              <td>
                <div class={`badge gap-2 badge-${badgeColor}`}>
                  {locale === 'ru' 
                    ? (item.priority.toLowerCase() === 'essential' ? 'Базовый' : item.priority.toLowerCase() === 'optional' ? 'Опционально' : item.priority.toLowerCase() === 'advanced' ? 'Продвинутый' : item.priority)
                    : item.priority}
                </div>
              </td>
              <td class={styles.checklistItemDescription} dangerouslySetInnerHTML={parseMarkdown(
                locale === 'ru' && props.section.slug
                  ? translateChecklistContent(props.section.slug, item.point, locale, 'details', item.details) || item.details
                  : item.details
              )}></td>
            </tr>
          )}
