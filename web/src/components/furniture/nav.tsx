...
export default component$(() => {
  ...
  const themes = [
    ...
    const isConfirmed = confirm(
      locale === 'ru' 
        ? 'Вы уверены, что хотите удалить все локальные данные? Это удалит ваш прогресс.'
                  ...
                  {data.value.map((item: Section, index: number) => (
                    <li key={`checklist-nav-${index}`} class={`hover:bg-${item.color}-600 hover:bg-opacity-15`}>
                      ...
                      <Icon color={item.color} class="mr-2" icon={item.icon} width={16} height={16}  />
                        {translateSectionTitle(item.title, locale)}
                      </a>
                    </li>
                  ))}
                </ul>
            ...
            <li>
              <div class="dropdown dropdown-end">
                <label tabIndex={0} class="btn btn-ghost btn-sm">
                  {locale === 'en' ? 'EN' : 'RU'}
                  <Icon icon="arrow-down" width={12} height={12} />
                </label>
                <ul tabIndex={0} class="dropdown-content menu bg-base-100 rounded-box z-[1] w-32 p-2 shadow">
                  <li><a onClick$={() => changeLocale('en')} class="cursor-pointer">English</a></li>
                  <li><a onClick$={() => changeLocale('ru')} class="cursor-pointer">Русский</a></li>
                </ul>
              </div>
            </li>
          </ul>
