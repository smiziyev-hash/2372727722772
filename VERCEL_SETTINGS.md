...
2. **Build Command**: `npm run build.server`
   - Эта команда сначала выполняет `qwik build` (генерирует клиентский манифест), затем собирает серверную часть для Vercel Edge
   - Важно: команда должна быть `npm run build.server`, а не просто `qwik build`
